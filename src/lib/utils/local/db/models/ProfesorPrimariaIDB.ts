import IndexedDBConnection from '../IndexxedDBConnection';
...
export class ProfesorPrimaria {
   ...
   */
  public async getAll(includeInactive: boolean = false): Promise<IProfesorPrimaria[]> {
    try {
   ...
   * @returns Profesor encontrado o null
   */
  public async getByNombreUsuario(nombreUsuario: string): Promise<IProfesorPrimaria | null> {
    try {
      const store = await IndexedDBConnection.getStore(this.storeName);
      ...
      console.error(`Error al obtener profesor con nombre de usuario ${nombreUsuario}:`, error);
      throw error;
    }
  }
   ...
   */
  public async searchByCriteria(filter: IProfesorPrimariaFilter): Promise<IProfesorPrimaria[]> {
    try {
      ...
      return allProfesores.filter(profesor => {
        ...
        if (filter.DNI_Profesor_Primaria && !profesor.DNI_Profesor_Primaria.includes(filter.DNI_Profesor_Primaria)) {
          matches = false;
        }
        
        if (filter.Nombres && !profesor.Nombres.toLowerCase().includes(filter.Nombres.toLowerCase())) {
          matches = false;
        }
        
        if (filter.Apellidos && !profesor.Apellidos.toLowerCase().includes(filter.Apellidos.toLowerCase())) {
          matches = false;
        }
  ...
  public async add(profesor: IProfesorPrimaria): Promise<string> {
    try {
      ...
      if (existingByDNI) {
        throw new Error(`Ya existe un profesor con el DNI ${profesor.DNI_Profesor_Primaria}`);
      }
      
      // Verificar que el nombre de usuario no exista
      const existingByUsername = await this.getByNombreUsuario(profesor.Nombre_Usuario);
      if (existingByUsername) {
        throw new Error(`Ya existe un profesor con el nombre de usuario ${profesor.Nombre_Usuario}`);
      }
      ...
      const store = await IndexedDBConnection.getStore(this.storeName, 'readwrite');
  ...
  public async update(profesor: IProfesorPrimaria): Promise<void> {
    try {
      ...
      if (!existing) {
        throw new Error(`No existe un profesor con el DNI ${profesor.DNI_Profesor_Primaria}`);
      }
      
      // Verificar nombre de usuario único si ha cambiado
      if (profesor.Nombre_Usuario !== existing.Nombre_Usuario) {
        const existingByUsername = await this.getByNombreUsuario(profesor.Nombre_Usuario);
        if (existingByUsername && existingByUsername.DNI_Profesor_Primaria !== profesor.DNI_Profesor_Primaria) {
          throw new Error(`Ya existe otro profesor con el nombre de usuario ${profesor.Nombre_Usuario}`);
        }
      }
      ...
      console.error(`Error al actualizar profesor con DNI ${profesor.DNI_Profesor_Primaria}:`, error);
      throw error;
    }
  }
  ...
  public async asignarAula(dniProfesor: string, idAula: number): Promise<void> {
    try {
      ...
      const aulaStore = await IndexedDBConnection.getStore('aulas', 'readwrite');
      ...
      console.error(`Error al asignar aula ${idAula} al profesor ${dniProfesor}:`, error);
      throw error;
    }
  }
   ...
   */
  public async cambiarContraseña(dniProfesor: string, nuevaContraseña: string): Promise<void> {
    try {
      const profesor = await this.getByDNI(dniProfesor);
      if (!profesor) {
        throw new Error(`No existe un profesor con el DNI ${dniProfesor}`);
      ...
      console.error(`Error al cambiar contraseña del profesor ${dniProfesor}:`, error);
      throw error;
    }
  }
   ...
   */
  public async actualizarFoto(dniProfesor: string, googleDriveFotoID: string): Promise<void> {
    try {
      const profesor = await this.getByDNI(dniProfesor);
      if (!profesor) {
        throw new Error(`No existe un profesor con el DNI ${dniProfesor}`);
      ...
      console.error(`Error al actualizar foto del profesor ${dniProfesor}:`, error);
      throw error;
    }
  }
          ...
          if (aulasRequest.result && aulasRequest.result.length > 0) {
            for (const aula of aulasRequest.result) {
              try {
                ...
                console.warn(`No se pudo desasignar el aula ${aula.Id_Aula}:`, error);
              }
            }
          }
   ...
   */
  public async login(nombreUsuario: string, contraseña: string): Promise<IProfesorPrimaria | null> {
    try {
      ...
      if (profesor.Estado === false) {
        throw new Error('La cuenta está inactiva. Contacte a un administrador.');
      }
...
//   }
   ...
   */
  public async sincronizarConServidor(onProgress?: (mensaje: string, progreso: number) => void): Promise<void> {
    try {
      ...
      for (const profesor of profesoresLocales) {
        ...
        if (onProgress) {
          contador++;
          onProgress(`Sincronizando profesor ${profesor.Nombres} ${profesor.Apellidos}...`, 
